function loadTempFeatures() {
	tmp.features = {
		...
		"time reversal": new Feature({
			name: "time reversal",
			req: function() { return new ExpantaNum("1e1e1000") },
			res: "distance",
			display: formatDistance,
			reached: function() { return player.tr.unl },
			progress: function () {
				if (player.options.featPerc=="logarithm") {
					return player.distance.max(1).log10().div(new ExpantaNum("1e1e9").log10());
				} else return ExpantaNum.div(new ExpantaNum("1e1e9"), player.distance).pow(-1)
			}
		}),
			...
			progress: function () {
				if (player.options.featPerc=="logarithm") {
					return player.distance
						...
						.max(1)
						.log10()
						.div(new ExpantaNum(COLLAPSE_UNL).times(tmp.collapse ? tmp.collapse.lrm : 1).log10().log10());
