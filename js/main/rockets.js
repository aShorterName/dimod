...
function getRocketGainMult() {
	...
	mult = mult.mul(mult.log10().pow(getReaEff()))
...
function updateTempRockets() {
	...
	tmp.rockets.layer = new Layer("rockets", tmp.rockets.canRocket, "normal", false, "rockets", true);
	tmp.rockets.gain=()=>{
		return player.distance.add(1).log10().add(1).log10().sub(1).floor().max(0).mul(getRocketGainMult())
	}
